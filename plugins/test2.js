...
let menuc5 = `╭━━〔${new Date().getHours() < 12 ? '*🌄 සුබ උදෑසනක් 🌄*' : '*🌛 සුබ රාත්‍රියක්  🌛*'}〕━━┈⊷
┃◈╭─────────────·๏
┃◈┃• *🧙‍♂️ 𝐙𝐀𝐍𝐓𝐀 × 𝐌𝐃 𝐎𝐅𝐂 🧙‍♂️*
┃◈└───────────┈⊷
╰──────────────┈
*◈╾──────${category}DEPLOY PRICE──────╼◈*\n\n> 𝚃𝙷𝙴 𝚉𝙰𝙽𝚃𝙰-𝚇𝙼𝙳 𝙲𝚁𝙴𝙰𝚃𝙴𝙳 𝙱𝚈 𝙼𝚁 𝚂𝚄𝚁𝙰𝙽𝙶𝙰 𝙼𝙾𝙳-𝚉  💖\n\n`;

for (let i = 0; i < commands.length; i++) {
  if (commands[i].category === 'ᴢᴀɴᴛᴀ-xᴍᴅ' && !commands[i].dontAddCommandList) {
    menuc5 += `┃◈│🔹 ${commands[i].commandName || 'Unnamed Command'}\n`;
  }
}

menuc5 += `╭━━━〔 🧙‍♂️ 𝐙𝐀𝐍𝐓𝐀 × 𝐌𝐃 𝐎𝐅𝐂 🧙‍♂️*〕━━━┈⊷
┃★╭──────────────
┃★│ 👑 Owner : *ᴍʀ ꜱᴜʀᴀɴɢᴀ*
┃★│ 📚 Version : *1.0*
┃★╰──────────────
